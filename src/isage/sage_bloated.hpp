#ifndef ISAGE_WTM_SAGE_BLOAT_H_
#define ISAGE_WTM_SAGE_BLOAT_H_
...
namespace isage {
  namespace wtm {
    ...
    public:
      SageVariationalHighMem<D, W, TopicType>() : background_(NULL), model_(NULL), corpus_(NULL) {
	own_background = false;
	own_topics = false;
      }
      SageVariationalHighMem<D, W, TopicType>(M* model, Corpus<D>* corpus, V* vocab, isage::wtm::SageTopicRegularization reg_how) :
	...
	for(int d = 0; d < num_docs_; d++) {
	  ...
	  var_assignment_params_.push_back(std::vector<std::vector<double> >(doc_multi.size(), 
									     std::vector<double>(num_topics_, 0.0))); 
	}
	expected_counts_.clear();
	// initialize expected counts
	const int vocab_size = const_cast<V*>(vocab_)->num_words();
	...
	//also, update the expected counts
        isage::util::linear_combination_in_first(&(expected_counts_[word]),
                                                 var_assignment_params_[doc_index][which_index],
	...
	for(const auto& doc_assigns : var_assignment_params_) {
	  ...
	  for(const auto& word_vec : doc_assigns) {
	    int t_id = 0;
	    WordCountType count = word_type_counts_[doc_id][d_w_id];
	...
	while(learn_iter++ < strategy.num_learn_iters && 
	  ...
	  // now do various printing options
	  {
		...
		model_changed || last_iter) {
	      ...
	      std::ostream& out_stream1 = assign_smart_wr.get(suff);
	      INFO << "Writing topic assignment output to " << assign_smart_wr.name();	    
